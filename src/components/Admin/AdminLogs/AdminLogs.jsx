...
import { 
    RiCpuLine, RiUserSettingsLine, RiTerminalBoxLine, 
    RiSearchLine, RiInformationLine, RiAlertLine, RiErrorWarningLine, RiDownload2Line, RiPieChartLine
} from "react-icons/ri";
...
const AdminLogsPage = () => {
    ...
    const chartData = [
        { name: 'Info', value: logs.filter(l => l.type === 'info').length, color: '#3b82f6' },
        { name: 'Warning', value: logs.filter(l => l.type === 'warning').length, color: '#f59e0b' },
        { name: 'Error', value: logs.filter(l => l.type === 'error').length, color: '#ef4444' },
    ];
    ...
    useEffect(() => {
        // Fuentes actuales
        const qDevice = query(collection(db, 'device_logs'), orderBy('timestamp', 'desc'), limit(100));
        const qUser = query(collection(db, 'system_logs'), orderBy('timestamp', 'desc'), limit(100));

        let deviceArr = [];
        let userArr = [];

        const mergeLogs = () => {
            const combined = [...deviceArr, ...userArr].sort((a, b) => 
        ...
        const unsubDevice = onSnapshot(qDevice, (snap) => {
            deviceArr = snap.docs.map(doc => ({ id: doc.id, origin: 'DEVICE', ...doc.data() }));
            mergeLogs();
        });
        ...
        const unsubUser = onSnapshot(qUser, (snap) => {
            userArr = snap.docs.map(doc => {
                ...
                // Si el tipo es 'success', lo agrupamos con 'info' para el filtro
        ...
        return () => { unsubDevice(); unsubUser(); };
    }, []);

    // Filtro inteligente
    const filteredLogs = logs.filter(log => {
        const matchesType = filter === 'all' || log.type === filter;
        const searchContent = `${log.action} ${log.details} ${log.deviceId || ''} ${log.userEmail || ''}`.toLowerCase();
    ...
    return (
        <div className={styles.container}>
            <div className={styles.topSection}>
                <header className={styles.header}>
                    <div className={styles.titleArea}>
                        <h2><RiTerminalBoxLine /> Consola Unificada</h2>
                ...
                <div className={styles.analyticsMini}>
                    <div className={styles.chartInfo}>
                        <h4><RiPieChartLine /> Distribución de Eventos</h4>
            ...
            <header className={styles.header}>
                <div className={styles.titleArea}>
                    <h2><RiTerminalBoxLine /> Centro de Mensajería Global</h2>
                    ...
                    {filteredLogs.map(log => (
                        <div key={log.id} className={`${styles.row} ${styles[log.type]}`}>
                            <div className={styles.colType}>
                                {log.type === 'error' ? <RiErrorWarningLine className={styles.err} /> : 
                                log.type === 'warning' ? <RiAlertLine className={styles.warn} /> : 
                                <RiInformationLine className={styles.info} />}
                            </div>
                            <div className={styles.colOrigin}>
                                {log.origin === 'DEVICE' ? 
                                    <span className={styles.badgeDevice}><RiCpuLine /> HARDWARE</span> : 
                                    <span className={styles.badgeUser}><RiUserSettingsLine /> USUARIO</span>
                                }
                            </div>
                            <div className={styles.colSubject}>
                                <code>{log.origin === 'DEVICE' ? log.deviceId : (log.userEmail || 'Admin')}</code>
                            </div>
                            <div className={styles.colAction}><strong>{log.action}</strong></div>
