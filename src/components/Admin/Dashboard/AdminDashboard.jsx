...
import { RiUserFollowLine, RiPulseLine, RiHistoryLine, RiAlertLine, RiExternalLinkLine, RiShieldUserLine } from "react-icons/ri";
import styles from './Admin.module.scss';
import toast from 'react-hot-toast';
import { Link } from 'react-router-dom';

const AdminDashboardPage = () => {
    const [stats, setStats] = useState({ users: 0, devices: 0, alerts: 0, activeAdmins: 0 });
    const [combinedLogs, setCombinedLogs] = useState([]);
    const [loading, setLoading] = useState(true);

    useEffect(() => {
        // Usamos onSnapshot para que el Dashboard sea TIEMPO REAL
        ...
        // Listener unificado de logs para el resumen
        const qLogs = query(collection(db, 'system_logs'), orderBy('timestamp', 'desc'), limit(8));
    ...
    if (loading) return <div className={styles.loader}><div className={styles.spinner}></div>Cargando Panel...</div>;
            ...
            <div className={styles.contentLayout}>
                ...
                <section className={styles.mainCard}>
                    <div className={styles.cardHeader}>
                        <h3><RiHistoryLine /> Actividad del Ecosistema</h3>
                        <Link to="/admin/logs" className={styles.viewMore}>Ver todo <RiExternalLinkLine /></Link>
                    </div>
                        ...
                        {combinedLogs.map(log => (
                            <div key={log.id} className={`${styles.logRow} ${styles[log.type]}`}>
                                ...
                                <div className={styles.logData}>
                                    ...
                                    <p>{log.details}</p>
                                    <small>{log.origin === 'DEVICE' ? `ID: ${log.deviceId}` : log.userEmail}</small>
                                </div>
                            </div>
                        ))}
                    </div>
